...
```{r}
x <- c(201706, 201703)
j <- str_pad(seq(12, 3, -3), 2, "left", "0")
for(i in 2016:1994) {
    x <- c(x, str_c(i, j))
}
fns <- file.path(ncuaDir, x, "FOICU.txt")
foicu <- ldply(fns, read.5300cr)
setDT(foicu, key = "CU_NUMBER")
foicu[, CYCLE_DATE := as.IDate(CYCLE_DATE, format = "%m/%d/%Y")]
foicu[CYCLE_DATE < "1998-06-30", CYCLE_DATE := as.IDate(sub("^00", "19", CYCLE_DATE))]
```
...
foicu.b[, CYCLE_DATE := as.IDate(CYCLE_DATE, format = "%m/%d/%Y")]
foicu.b[CYCLE_DATE < "1998-06-30", CYCLE_DATE := as.IDate(sub("^00", "19", CYCLE_DATE))]
```
```{r}
